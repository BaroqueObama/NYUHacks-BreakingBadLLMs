...
import whisper.tokenizer  # or the appropriate submodule where Tokenizer is defined

original_encode = whisper.tokenizer.Tokenizer.encode

def patched_encode(self, text, **kwargs):
    # Ensure disallowed_special is always an empty tuple
    kwargs.setdefault("disallowed_special", ())
    return original_encode(self, text, **kwargs)

whisper.tokenizer.Tokenizer.encode = patched_encode
...
def evaluate(hparams_file, run_opts, overrides):
    ...
    for k in test_datasets.keys():  # keys are test_clean, test_other etc
        ...
        aA = test_datasets[k]
        print("aA Worked")
        bB = hparams["test_dataloader_opts"]
        print("bB Worked")
        
        target_brain.evaluate(
            aA,
            test_loader_kwargs=bB,
