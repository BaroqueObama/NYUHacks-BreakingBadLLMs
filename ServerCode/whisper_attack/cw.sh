...
python run_attack.py attack_configs/cw.yaml --root=/workspace/pythonny --data_csv_name=$DATA --model_label=medium --tokenizer_name=gpt2 --nb_iter=$NBITER --load_audio=$LOAD --seed=$SEED --attack_name=$NAME --eps=$EPS --const=$CST --max_decr=$MAXDECR --lr=$LR --confidence=$CONF --decrease_factor_eps=$DECRFACTOR
